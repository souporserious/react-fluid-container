      ...
      },
      {
        ...
        panel: (
          <div style={{ padding: 0.1 }}>
            <p>Some cool content for accordion two.</p>
            <p>Some extra cool content for accordion two 💃</p>
          </div>
        ),
      },
      {
      ...
      <AriaManager type="tabs">
        <div className="tab-set">
            ...
            {tabs.map(({ id, title }) => (
              <AriaTab key={id} id={id}>
                {(props, isActive) => (
                  <div
                    {...props}
                    className={`tab-list-item ${isActive ? 'is-active' : ''}`}
                  >
                    {title}
                  </div>
                )}
          ...
          <div className="tab-panels">
            <FluidContainer height={height} style={{ overflow: 'hidden' }}>
              <div>
                {tabs.map(({ id, panel }) => (
                  <AriaPanel key={id} controlledBy={id} className="tab-panel">
                    {panel}
                  </AriaPanel>
                ))}
              </div>
...
class App extends Component {
  render() {
    return (
      <div>
        <FluidContainer tag="div" height="auto" className="site-header">
          <h1 contentEditable>
